...
from apps.user.models import QueuePaymentVerificationUser, DepositVoucherUser, DepotUser
...
def process_payment_queue():
    try:
        ...
        pending_payments = QueuePaymentVerificationUser.objects.filter(status='P')
            ...
            for queue_payment in pending_payments:
                try:
                    ...
                    if response.status_code == status.HTTP_200_OK:
                        ...
                        deposit_user.modified_at = get_timezone()
                        deposit_user.save()
                        print("deposit user  success....")
